...
cat <<EOF>> .bashrc 
alias crm='docker container rm -f $(docker container ls -aq)' >> ~/.bashrc
alias irm='docker image rm -f $(docker image ls -aq)' >> ~/.bashrc
alias cls='docker container ls -a' >> ~/.bashrc
EOF
printf "\n"
sleep 3
